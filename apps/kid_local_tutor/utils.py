...
def format_prompt(
    ...
    persona: str = "You are a helpful tutor.",
    system_instructions: str | None = None,
) -> str:
    """Combine system instructions, persona, retrieved context, and the user query into a prompt.

    The tutor enforces a strict provenance contract:

    - Every factual/declarative sentence must end with bracket citations like [1] [2].
    - Citation numbers must refer to the numbered context passages.
    - If the answer cannot be supported by the provided context, say:
      "I do not know from this local library."
    - End with a Sources block mapping [n] to the provenance lines provided in the context.
    """

    default_system = (
        "You are an offline tutor. You must follow the provenance contract.
"
        "Rules:
"
        "1) Every factual or declarative sentence MUST end with one or more citations like [1] [2].
"
        "2) Citations MUST refer to the numbered context passages provided below.
"
        "3) If you cannot support a sentence from the context, do not guess. Say: "
        ""I do not know from this local library."
"
        "4) End your response with a Sources section that lists each cited [n] with the exact "
        "provenance lines from the context.
"
        "5) Keep sentences short and clear.
"
    )
    sys_text = (system_instructions or default_system).strip()

    # Number the context passages so citations can resolve deterministically.
    numbered_context_parts: List[str] = []
    for i, c in enumerate(context, start=1):
        c = (c or "").strip()
        if not c:
            continue
        numbered_context_parts.append(f"[CONTEXT {i}]
{c}")
...
".join(numbered_context_parts) if numbered_context_parts else "(no context)",
        "
